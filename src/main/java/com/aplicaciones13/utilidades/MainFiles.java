...
public class MainFiles {
    ...
    //Semilla de security
    private static final String SEMILLA = "JRFG.Admin.2483700";
    //Path del archivo
    private String path;
    ...
    /** Metodo para obtener el valor desencriptado.
     *
     * Crea el objeto encriptador
     * Agrega la semilla
     * Devuelve el valor encriptado
     *
     * @return
     */
    public String getTextDesencriptado() {
        StandardPBEStringEncryptor standardPBEStringEncryptor = new StandardPBEStringEncryptor();
        standardPBEStringEncryptor.setPassword(SEMILLA);

        try {
            return standardPBEStringEncryptor.decrypt(getText());
        } catch (Exception e) {
            return "";
        }
    }

    /** Metodo para encripar un texto y poner en el codigo.
     *
     * Crea el objeto de encripcion
     * Pone la misma semilla
     * Obtiene el valor desencriptado.
     *
     * @param cadena
     */
    public void setTextEncriptado(String cadena) {
        StandardPBEStringEncryptor standardPBEStringEncryptor = new StandardPBEStringEncryptor();
        standardPBEStringEncryptor.setPassword(SEMILLA);
        setText(standardPBEStringEncryptor.encrypt(cadena));
    }
    ...
    public static String escribirLogJS(boolean interruptor, Object... palabras) {
        if (interruptor)
            try {
                ...
                fileWriter.write(String.valueOf(new Date()) + " " + addSt(palabras) + "\n");
                fileWriter.flush();
                fileWriter.close();
            } catch (IOException e) {
